...
- Fixed insecure default JWT secret rejection at startup in the Agent Builder.
- Ensured Eppo control panel renders correctly above Polly in the frontend.
- Standardized dropdown width and height in Bedrock and Azure model config in the Playground.
- Addressed issue with missing default role ID for workspace invites when RBAC is not enabled.
- Updated the prompt owners dialog in the frontend to be fully backwards compatible.
- Added detailed per-annotator feedback in experiments CSV export.
- Added a new API endpoint for GET /me/slack_id in the Agent Builder.
- Implemented Slack authorization in the Agent Builder with is_user_permitted_for_agent feature.
- Added strict:true to prebuilt evaluator schemas and sync on model switch in the frontend.
- Fixed potential HTTP auth leak in message_received.py.
- Improved agent proactivity by reading the current run in Smith-Polly.
- Added flag to disable query shadowing.
- Fixed search issue in the member owners dialog on the frontend.
- Added CRUD API for forge configurations.
- Added real-time sub-agent streaming UI in the Agent Builder.
- Fixed raw error message display on the home page for tracing projects.
- Stopped auto-saving in the identity modal channels in the Agent Builder.
...
- Fixed the rejection of insecure default JWT secrets at startup for Agent Builder.
- Ensured the Eppo control panel renders correctly over Polly in the frontend.
- Standardized dropdown width and height in Bedrock and Azure model configuration in the playground.
- Updated the dialog for prompt owners to be fully backwards compatible.
- Added per-annotator feedback details in experiments CSV export.
- Improved OAuth token handling in Agent Builder to dynamically retrieve the token.
- Enhanced the UI for the prompt hub and evaluator to use model schemas with strict validation.
- Addressed a bug that capped at 100 files included in the backfill.
- Added a statement timeout setting at the database level for improved performance.
- Integrated new forge settings page with an experimental UI.
- Enhanced Slack integration in the Agent Builder with delete user integrations on disconnect and added a GET /me/slack_id endpoint.
- Added authorization middleware for GO in ABAC support.
- Improved various UI and frontend elements including tooltips, color palettes, and chip popover redesigns.
- Fixed the inbox sorting and long message wrapping in the Agent Builder.
- Implemented a real-time sub-agent streaming UI in Agent Builder.
- Updated the UI to hide raw error messages on the home page for tracing projects.
