...
Prompt owners give you fine-grained control over who can tag commits and delete a specific prompt. This is useful for production promotion flows where you want to limit which team members can update a tag like `production` to point to a new commit.

### Access modes

Each prompt has two access modes, configured under **Access and Permissions**:

- **Workspace authorized users** (default) — any workspace user with `prompts:tag` permission can create, update, and delete tags and delete the prompt.
- **Owners only** — only users in the prompt's owners group can create or update commit tags and delete the prompt.

The prompt creator is automatically added as an owner when the prompt is created.
...
<Warning>
If you remove yourself from the owners group, you immediately lose the ability to manage owners or switch the prompt back to anyone mode. Only another owner can restore your access.
</Warning>

When owners only mode is active, only an owner can add or remove other owners or disable owners only mode.
